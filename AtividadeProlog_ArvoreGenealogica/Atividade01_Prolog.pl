...
tio(X,Y)  :- mae(S, Y), irmao(S, X); pai(S, Y), irmao(S, X), !. % funciona para achar tio, 
tia(X,Y)  :- mae(S, Y), irma(S, X); pai(S, Y), irma(S, X), !. % funciona para achar tia,
...
%falta achar descendentes e ascendentes
