...
def breadth_first_graph_search(problem):
    ...
    if problem.goal_test(node.state):
        return node
    frontier = deque([node]) # enfilera o nó na fila de fronteira gf2
    explored = set()
    while frontier:
        node = frontier.popleft() # desinfilera o primeiro nó da fila de nós fronteira
        explored.add(node.state) # adiciona o estado do nó fronteira explorado ao set de nós explorados 
        for child in node.expand(problem): # o filho do nó expandido vai para a fila de nós fronteira, 
            #se ele for a meta retorna esse nó
            if child.state not in explored and child not in frontier:
